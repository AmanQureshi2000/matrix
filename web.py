...
try:
    ...
    def lu(*args, **kwargs): raise ImportError("scipy not available")
    def expm(*args, **kwargs): raise ImportError("scipy not available")
    def logm(*args, **kwargs): raise ImportError("scipy not available")
    def svd(*args, **kwargs): raise ImportError("scipy not available")
    def qr(*args, **kwargs): raise ImportError("scipy not available")
    def schur(*args, **kwargs): raise ImportError("scipy not available")
    def hessenberg(*args, **kwargs): raise ImportError("scipy not available")
    def solve(*args, **kwargs): raise ImportError("scipy not available")
    def lstsq(*args, **kwargs): raise ImportError("scipy not available")
    def pinv(*args, **kwargs): raise ImportError("scipy not available")
    def pdist(*args, **kwargs): raise ImportError("scipy not available")
    def squareform(*args, **kwargs): raise ImportError("scipy not available")
    def describe(*args, **kwargs): raise ImportError("scipy not available")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body.dark-theme .header { border-bottom-color: #3d3d5c; }
        .header h1 {
            font-size: 28px;
            font-weight: 700;
            color: #2c3e50;
        }
        body.dark-theme .header h1 { color: #cdd6f4; }
        ...
        body.dark-theme .panel { background: #2d2d44; }
        ...
        body.dark-theme .matrix-header h3 { color: #cdd6f4; }
        .matrix-controls {
            display: flex;
            align-items: center;
            gap: 6px;
        ...
        .matrix-controls label { font-size: 12px; margin: 0 2px; }
        ...
        body.dark-theme .tabs { border-bottom-color: #3d3d5c; }
        .tab-btn {
            background: #ecf0f1;
            border: none;
            padding: 6px 14px;
        ...
        body.dark-theme .tab-btn:hover { background: #4d4d6c; }
        .tab-btn.active {
            background: #ffffff;
            color: #2c3e50;
        ...
        .tab-content { flex: 1; min-height: 200px; }
        .tab-pane {
            display: none;
            flex-wrap: wrap;
            gap: 6px;
        ...
        .tab-pane .btn { flex: 0 0 auto; }
        .right-panel { display: flex; flex-direction: column; }
        .result-group { flex: 1; display: flex; flex-direction: column; }
        .result-group h3 {
            font-size: 14px;
            margin-bottom: 8px;
            color: #2c3e50;
        }
        body.dark-theme .result-group h3 { color: #cdd6f4; }
        .result-display {
            flex: 1;
            min-height: 300px;
            font-family: 'Courier New', monospace;
        ...
        body.dark-theme .result-info { color: #7f8c9e; }
        .result-actions {
            display: flex;
            gap: 8px;
            margin-top: 6px;
        ...
        .help-content h3 { margin-top: 15px; margin-bottom: 8px; }
        .help-content ul { margin-left: 20px; margin-bottom: 10px; }
        .help-content li { margin-bottom: 4px; }
        @media (max-width: 1024px) { .main-layout { grid-template-columns: 1fr; } }
        @media (max-width: 600px) {
            .header h1 { font-size: 20px; }
            .matrix-header { flex-direction: column; align-items: stretch; }
...
<body>
    ...
    <script>
        ...
        document.getElementById('helpBtn').onclick = function() { modal.style.display = 'block'; }
        document.querySelector('.close').onclick = function() { modal.style.display = 'none'; }
        window.onclick = function(event) {
            if (event.target == modal) { modal.style.display = 'none'; }
        }

        document.getElementById('resizeA').addEventListener('click', function() { resizeMatrix('A'); });
        document.getElementById('resizeB').addEventListener('click', function() { resizeMatrix('B'); });
        ...
        function getMatrixData(name) {
            return document.getElementById('matrix' + name).value;
        }
        ...
        async function operate(operation) {
            ...
            if (matrixB.trim()) { payload.matrix_b = matrixB; }

            if (operation === 'power' || operation === 'elementwise_power') {
                const power = prompt('Enter power:');
                if (power === null) return;
                const p = parseFloat(power);
                if (isNaN(p)) { alert('Please enter a valid number'); return; }
                payload.power = p;
            }
            ...
            if (operation === 'solve' || operation === 'least_squares') {
                ...
                if (b === null) return;
                if (!b.trim()) { alert('Vector b is required'); return; }
                payload.b = b;
            }
        ...
        function promptPower() {
            ...
            if (power !== null) {
                ...
                if (!isNaN(p)) { operate('power'); }
                else { alert('Please enter a valid integer'); }
            }
        }

        function promptSolve() {
            const b = prompt('Enter vector b (space/comma separated):');
            if (b !== null && b.trim()) { operate('solve'); }
        }

        function promptLeastSquares() {
            const b = prompt('Enter vector b (space/comma separated):');
            if (b !== null && b.trim()) { operate('least_squares'); }
        }

        document.addEventListener('keydown', function(e) {
            if (e.ctrlKey && e.key === 'c') { e.preventDefault(); copyResult(); }
...
def operate_matrices():
    ...
    if not SCIPY_AVAILABLE:
        return jsonify({'success': False, 'error': 'Scipy libraries not available. Please contact administrator.'})
